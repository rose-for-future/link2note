---
name: 信息处理
description: 给一个链接（已支持：抖音/小红书/播客/B站/公众号/GitHub；微博/知乎规划中暂不可用）就自动抓取内容、视频音频本地转写、识别图片、由我做要点总结，产出「文字稿.md + 总结.md」到你的 Obsidian 知识库。当用户粘贴上述平台链接并希望提取/转写/总结/存档内容时使用。
---
...
用户给出 **抖音 / 小红书 / 播客 / B站 / 公众号 / GitHub** 链接，想要：转文字、提炼要点、存进知识库。
（微博 / 知乎 规划中，暂不可用——贴这些链接 router 能识别但会提示 fetcher 尚未实现。）
